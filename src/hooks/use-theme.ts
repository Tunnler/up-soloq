...
export const useTheme = (defaultTheme?: Theme) => {
  const [theme, setTheme] = useState<Theme>(() => {
    ...
    return storedTheme || (defaultTheme ?? ThemeProps.dark); // Cambiado a ThemeProps.dark
  });
  ...
  useEffect(() => {
    _setTheme(theme);
  }, [theme]); // Agregado el array de dependencias para evitar loops infinitos en useEffect
